...
df3 <- df^3
df3 <- df3[,-c(2,3,10)]
colnames(df3) <- paste0(colnames(df3), '_cbd')
...
```{r standardize}
norm.value <- preProcess(df, method = c("center", "scale"))

df <- predict(norm.value, df)
df_ROSE <- predict(norm.value, df_ROSE)
df_BOTH <- predict(norm.value, df_BOTH)
df_OVER <- predict(norm.value, df_OVER)
df_UNDER <- predict(norm.value, df_UNDER)
