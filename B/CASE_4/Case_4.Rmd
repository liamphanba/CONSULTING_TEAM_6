...
mod <- lm(DIFF ~ ., data = subset(teaching, select = -c(PRE, POST, ATT)))
mod <- lm(ATT ~., data = subset(teaching, select = -c(PRE, POST, DIFF)))
summary(mod)
plot(mod)
...
I guess we can assum normality as qqplot in bootstrap. 
