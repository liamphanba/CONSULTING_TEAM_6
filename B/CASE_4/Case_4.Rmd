...
teaching$TAUGHT <- factor(ifelse(teaching$PREF == 'N' | teaching$GROUP == 'Control', 'TR', teaching$PREF), level = c('TR', 'A', 'T', 'K', 'V'), labels = c('Traditional', 'Auditory', 'Tactile', 'Kinesthetic', 'Visual'))
...
```{r}
library(fastDummies)
teaching <- dummy_columns(teaching, select_columns = c('GROUP', 'SESSION', 'PREF', 'GENDER', 'SLEVEL', 'TAUGHT'), remove_selected_columns = TRUE, remove_first_dummy = TRUE)
```
...
teaching_DID <- subset(teaching_DID, select = -c(POST, PRE, DIFF))

mod <- lm(OUTCOME ~ ., teaching_DID)
summary(mod)
```
...
Shows which variables have higher importance but we don't 

# Selection of variables

[selections](https://cran.r-project.org/web/packages/olsrr/vignettes/variable_selection.html) [more selections and lasso regression](https://rpubs.com/fractalbass/601525)

## forward

```{r}
mod_min <- lm(OUTCOME ~ 1, teaching_DID)
step(mod_min, direction='forward', scope=formula(lm(OUTCOME ~ ., data = teaching_DID)))
mod2 <- lm(formula = OUTCOME ~ POST_ind + ATT + SESSION_S3 + PREF_Visual, 
    data = teaching_DID)
summary(mod2)
plot(mod2)
plot.glmSim(mod2)
...
s
mod3 <- lm(OUTCOME ~ ATT + SESSION_S3 + PREF_Visual + POST_ind, data=teaching_DID)
summary(mod3)
plot(mod3)
plot.glmSim(mod3)
```
...
```{r}
models <- leaps::regsubsets(OUTCOME~., data = teaching_DID, nvmax = 20,
                     method = "seqrep")
s <- summary(models)

s$which[which.max(s$adjr2),]
mod45 <- lm(OUTCOME ~ ATT + SESSION_S2 + SESSION_S3 + `PREF_No Preference` + PREF_Visual + TAUGHT_Tactile + TAUGHT_Visual + POST_ind, teaching_DID)
summary(mod45)
```

No models fitted as it doesn't make sense
...
```{r}
# model due to sense
mod4 <- lm(OUTCOME ~ ATT + GROUP_Experiment + TAUGHT_Auditory + TAUGHT_Tactile + TAUGHT_Kinesthetic + TAUGHT_Visual, teaching_DID)
...
```{r}
# model due to sense
mod4 <- glm(DIFF ~ ATT + GROUP_Experiment + TAUGHT_Auditory + TAUGHT_Tactile + TAUGHT_Kinesthetic + TAUGHT_Visual, teaching, family=gaussian(link='identity'))
