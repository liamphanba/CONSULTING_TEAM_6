...
AS forward, backward and combined variable seleciton give the same model we fit it. 

```{r}
mod2 <- lm(formula = OUTCOME ~ POST_ind + ATT + SESSION_S3 + PREF_Visual + GROUP_Experiment*POST_ind,
    data = teaching_DID)
summary(mod2)
plot(mod2)
plot.lmSim(mod2)
...
There is a no impact of the experiment per se
...
mod4 <- lm(OUTCOME ~ ATT + GROUP_Experiment + PREF_N + PREF_T + PREF_K + PREF_V, teaching_DID)
