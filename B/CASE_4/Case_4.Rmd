...
```{r}
teaching$SESSION <- paste0(as.character(teaching$SESSION), '_', as.character(teaching$GROUP))
teaching$ID <- 1:nrow(teaching)
teaching_DID <- teaching %>% slice(rep(1:n(), each = 2))
teaching_DID$POST_ind <- rep(c(0,1),nrow(teaching)) 
teaching_DID$OUTCOME <- ifelse(teaching_DID$POST_ind == 1, teaching_DID$POST, teaching_DID$PRE)
...
```{r}
mod <- lm(OUTCOME ~ ATT + GROUP_Experiment + SESSION_S1_Experiment + SESSION_S2_Experiment + SESSION_S3_Experiment + SESSION_S4_Experiment + SESSION_S2_Control + SESSION_S3_Control + SESSION_S4_Control + `PREF_No Preference` + PREF_Tactile + PREF_Kinesthetic + PREF_Visual + GENDER_Female + SLEVEL_Highschool + ID + POST_ind + GROUP_Experiment * POST_ind, teaching_DID)
summary(mod)
plot(mod)
plot.lmSim(mod)
```
...
mod23 <- lm(formula = OUTCOME ~ ATT + GROUP_Experiment + SESSION_S2_Control + 
    PREF_Visual + POST_ind + GROUP_Experiment:POST_ind, data = teaching_DID)
summary(mod23)
plot(mod23)
plot.lmSim(mod23)
```
...
>>>>>>> 3f51e32cdf0f4d45395db3dcbc87f58b8a6f9aed
